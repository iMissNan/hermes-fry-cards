...
def handle_approval_card_action_patched(
    ...
    operator = getattr(event, "operator", None)
    open_id = str(getattr(operator, "open_id", "") or "")
    # Fail-closed: any error inside the gate must NOT grant the bypass.
    # (The gate itself returns True for an unconfigured deployment — that is
    # upstream's documented "no admins == unrestricted" semantics, not ours.)
    try:
        authorized = bool(self._is_interactive_operator_authorized(open_id))
    except Exception:
        _logger.warning(
            "[fry-cards] approval gate raised; denying click (fail-closed)",
            exc_info=True,
        )
        authorized = False
